...
if(isset($_POST["query"]))
{
	...
	if($_POST["query"] != '')
	{
		...
		$query = "
		SELECT id, name, username FROM volunteer WHERE name LIKE :name OR username LIKE :username ORDER BY id DESC ";
		...
		foreach($result as $row)
		{
			$data[] = array(
				...
				'username'	=>	str_ireplace($replace_array_1, $replace_array_2, $row["username"])
			);
		}
	...
	else
	{
		...
		$query = "
		SELECT id, name, username FROM volunteer ORDER BY id DESC ";

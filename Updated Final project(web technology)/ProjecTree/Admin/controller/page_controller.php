...
if(isset($_POST["query"]))
{
	...
	$connect = new PDO("mysql:host=localhost; dbname=tpc", "root", "");
	...
	if($_POST["query"] != '')
	{
		...
		$query = "
		SELECT id, name, variant, stock, planted FROM tree WHERE name LIKE :name OR variant LIKE :variant OR stock LIKE :stock OR planted LIKE :planted ORDER BY id DESC ";
		...
		foreach($result as $row)
		{
			$data[] = array(
				...
				'variant'	=>	str_ireplace($replace_array_1, $replace_array_2, $row["variant"]),
				'stock'			=>	$row["stock"],
				'planted'			=>	$row["planted"]
			);
		}
	...
	else
	{
		...
		$query = "
		SELECT id, name, variant, stock, planted FROM tree ORDER BY id DESC ";
